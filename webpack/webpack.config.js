...
module.exports = {
  entry: {
    ...
    //如果name下指定的模块是数组，则会自动生成一个id为0的module，依次加载数组中的所有module，最后一个module会被chunk返回
    page1: './src/page1.js',
    page2: './src/page2.js',
    vendor: ['./src/util.js']
  },
  ...
  plugins: [
    ...
    new CommonsChunkPlugin({
      ...
      //应该被提取公共部分的chunk，默认是所有entry chunk
    ...
    new CommonsChunkPlugin({
      //如果声明的name不存在于entry中，则相当于是声明了一个entry chunk
      //如果声明的name存在于entry中，则相当于声明这个chunk是公共的，chunks中所选的chunk不会再包含公共chunk中的module
      //chunks中如果包含runtime，都会被移到common chunk中
      name: 'vendor',
      filename: '[name].chunk.js',
      //Infinity可以防止其他模块被并引入公共chunk
      minChunks: Infinity
    })
  ]
};